...
from cogs.games._classes import BaseHand, Deck, BaseCard, NAMED_HAND, SUITS, UNAMED
from cogs.utils import helpers, render
from cogs.utils.constants import coin_emoji
from cogs.utils.context import Context, tick
from cogs.utils.formats import number_suffix, RevDict
...
def item_by_count(array: np.ndarray, n: int) -> np.ndarray[int]:
    """Returns the `n` common element from an iterable."""
    counts = np.bincount(array.flatten())
    if n not in counts:
        # placeholder
        return np.zeros(10, dtype=int)  # noqa
    return np.argwhere(counts == n).flatten()  # noqa
    ...
    def evaluate(self, community_arr: np.ndarray) -> HandResult:
        ...
        if combs is None:
            ...
            return HandResult(
                ...
                value=np.max(self.card_arr[:, 0]) // 16
            )
        ...
        return HandResult(
            name=name,
            cards=[Card(suit=x[1], value=x[0])
                   for x in combs.all_combos[0, np.argmax(combs.ranking), :, :]],
            value=best_comb.rank
        )

    def hand_value(self, community_arr: np.ndarray) -> Optional[CombResult]:
        """Returns the value of the player's hand.
...
class Ranker:
    ...
    def rank_all_hands(cls, hand_combos: np.ndarray, return_all: bool = False) -> RankingItem | np.ndarray[RankingItem]:
        ...
        rank_res_arr: np.ndarray[RankingItem] = np.zeros(shape=(hand_combos.shape[1], hand_combos.shape[0]), dtype=RankingItem)  # noqa
    ...
    def rank_one_hand(cls, hand_combos: np.ndarray, group_by: bool = False) -> np.ndarray[RankingItem]:
        ...
        rank_arr: np.ndarray[RankingItem] = np.zeros(num_combos.shape[0], dtype=RankingItem)  # noqa
    ...
    @staticmethod
    def straight_flush_check(num_combos: np.ndarray, rank_arr: np.ndarray, straight_arr: bool, suit_arr: bool):
    ...
    @staticmethod
    def four_of_a_kind_check(num_combos: np.ndarray, rank_arr: np.ndarray):
    ...
    @staticmethod
    def full_house_check(num_combos: np.ndarray, rank_arr: np.ndarray):
    ...
    @staticmethod
    def flush_check(num_combos: np.ndarray, suit_combos: np.ndarray, rank_arr: np.ndarray, suit_arr: bool):
    ...
    @staticmethod
    def straight_check(num_combos: np.ndarray, rank_arr: np.ndarray, straight_arr: bool):
    ...
    @staticmethod
    def three_of_a_kind_check(num_combos: np.ndarray, rank_arr: np.ndarray):
    ...
    @staticmethod
    def two_pairs_check(num_combos: np.ndarray, rank_arr: np.ndarray):
    ...
    @staticmethod
    def one_pair_check(num_combos: np.ndarray, rank_arr: np.ndarray):
    ...
    @staticmethod
    def high_card_check(num_combos: np.ndarray, rank_arr: np.ndarray):
...
class Player:
    ...
    def __init__(self, member: discord.Member, stack: int):
...
class Pot:
    ...
    def __init__(self, amount: int, players: list[Player] = []):  # noqa
...
class Table:
    ...
    def __init__(
        ...
        # Utils
        self.economy: Economy = ctx.bot.get_cog('Economy')  # noqa
        ...
        self.analysis: list[tuple[dict[str, float], dict[int, dict[str, float]]]] = []  # noqa

        self.view: TableView = TableView(table=self)

        # Event Loop
        self.loop: asyncio.AbstractEventLoop = asyncio.get_event_loop()
        self.running_autoplay_loop: Optional[asyncio.Task] = None

    def __repr__(self) -> str:
        return f'Table(state={self.state} host={self.host} players={len(self.players)} max_players={self.max_players})'

    @property
    def playing_players(self) -> list[Player]:
        """Returns a list of players that are still playing"""
    ...
    def AllIn(self):
        ...
        contribution = min(player.stack, self.pot.amount)

        player.stack -= contribution
        self.pot += contribution

        # Create a new side pot if necessary
        if self.side_pots:
            self.side_pots[-1] += contribution
        else:
            self.side_pots = [Pot(amount=contribution, players=[p for p in self.playing_players if not p.all_in])]

        self.switch_player(by_raise=True)
    ...
    def Call(self):
        ...
        # Update the last side pot with the contribution
        if self.side_pots:
            self.side_pots[-1] += contribution

        # Check if a new side pot needs to be created
        if call_amount > contribution:
            self.side_pots.append(Pot(amount=call_amount - contribution, players=self.playing_players))

        self.Check()

    def Raise(self, amount: int):
    ...
    def Bet(self, amount: int):
        """Bets the amount for the current player."""
        self.Raise(amount)
        self.Check()
    ...
    def add_player(self, member: discord.Member, stack: int):
    ...
    async def remove_player(self, member: discord.Member):
        ...
        if stack_left > 0:
            ...
            await self.ctx.bot.pool.execute(query, stack_left, member.id, self.ctx.guild.id)
    ...
    def switch_player(self, by_raise: bool = False):
    ...
    async def autoplay(self, player: Player):
        """
        Automatically plays for a player if they take too long for their turn.

        This function assumes that the player has taken too long and needs
        to be auto-played.
        """
    ...
    async def start_timer(self, player: Player):
    ...
    def game_is_over(self) -> bool:
        ...
        return (
                self.state == TableState.FINISHED
                or (len(self.community_arr) == 5 and all(player.checked for player in self.playing_players))
    ...
    def _simulation_preparation(self, num_scenarios: int | Literal['all']) -> tuple[Optional[np.ndarray], np.ndarray]:
    ...
    def _simulate_calculation(self, community_cards: np.ndarray, undrawn_combos: np.ndarray) -> np.ndarray[RankingItem]:
        ...
        np.ndarray
            The result array.
        """
        res_arr: np.ndarray[RankingItem] = np.zeros(shape=(len(undrawn_combos), len(self.players)), dtype=RankingItem)  # noqa
        if len(self.players) >= 2:
    ...
    def build_embed(self, with_autoplay: bool = False) -> discord.Embed:
        ...
        embed.description += (
            ...
            f'**Pot:** {coin_emoji} `{self.pot}`'
        )

        for i, side_pot in enumerate(self.side_pots, start=1):
            embed.description += f'**Side Pot *#{i}*:** {coin_emoji} `{side_pot}`\n'
    ...
    def _build_stopped_embed(self, embed: discord.Embed):
    ...
    def _build_running_embed(self, embed: discord.Embed, with_autoplay: bool = False):
        for index, player in enumerate(self.players, 1):
            name_parts = [f'Seat #{index}', player.member.display_name]
            text = f'**Stack:** {coin_emoji} `{player.stack}`\n'

            if self.state == TableState.RUNNING:
                if index - 1 == self.player_index:
                    name_parts.insert(0, str(discord.PartialEmoji(
                        name='vegarightarrow', id=1066024566046076938, animated=True)))

                blind = 'BB' if index == self.blind_index[1] + 1 else 'SB' if index == self.blind_index[0] + 1 else None
                if blind is not None:
                    name_parts.append(blind)

                text += f'**Current Bet:** {coin_emoji} `{player.bet}`\n'
            ...
            if player.folded:
                ...
                if self.state == TableState.FINISHED:
                    ...
                    name_parts.append(f'{coin_emoji} {won_lost_chips}')
    ...
    def _add_players_raw_to_embed(self, embed: discord.Embed):
        for index, player in enumerate(self.players, 1):
            name_parts = [f'Seat #{index}', player.member.display_name]
            text = f'**Stack:** {coin_emoji} `{player.stack}`\n'
    ...
    def _add_community_cards_to_embed(self, embed: discord.Embed):
...
class TableView(discord.ui.View):
    ...
    def __init__(self, table: Table):
        ...
        self.analysis = AnalysisButton(table=self.table)
        self.leave = LeaveButton(table=self.table)
    ...
    async def on_timeout(self) -> None:
        ...
        with suppress(discord.HTTPException):
            await self.table.message.reply(f'{tick(False)} The table has been closed due to inactivity.')
    ...
    def _update_buttons_not_running(self):
        for child in self.children:
            assert isinstance(child, discord.ui.Button)

            if child.label in ('Fold', self.check_call.label, 'Raise', 'All In', 'My Hand', 'Bet'):
                self.remove_item(child)
                continue

            child.disabled = False

            if child.custom_id == 'start_next_round':
                child.label = 'Next Round' if self.table.state == TableState.FINISHED else 'Start'

        if not any(child.label == 'Leave' for child in self.children):
            self.add_item(self.leave)

        if self.table.state == TableState.FINISHED:
            self.add_item(self.analysis)
        if self.table.state == TableState.PREPARED:
            self.remove_item(self.analysis)
    ...
    def _update_buttons_running(self):
        is_running = self.table.state == TableState.RUNNING
        self.start_next_round.disabled = not self.table.state == TableState.PREPARED
        self.fold.disabled = not is_running
        self.check_call.disabled = not is_running
        self.all_in.disabled = not is_running
        self.my_hand.disabled = not is_running

        # Big/Small Blind can't raise/bet in the first round
        is_first_round_and_blind = (
                len(self.table.community_arr) == 0 and self.table.player_index in self.table.blind_index
        ) if is_running else True
        self.raise_bet.disabled = is_first_round_and_blind
        self.raise_bet.label = 'Bet' if all(
            player.bet <= self.table.big_blind for player in self.table.playing_players) else 'Raise'

        # Setting the check/call button
        is_check = self.table.players[self.table.player_index].bet == max([player.bet for player in self.table.players])
        call_amount = max([player.bet for player in self.table.players]) - self.table.players[
            self.table.player_index].bet
        self.check_call.label = 'Check' if is_check else f'Call ({call_amount} Chips)'
        self.check_call.emoji = None if is_check else coin_emoji
        self.check_call.style = discord.ButtonStyle.grey if is_check else discord.ButtonStyle.green

        if not is_check and self.table.players[self.table.player_index].stack < call_amount:
            self.check_call.disabled = True
            self.check_call.style = discord.ButtonStyle.grey

    # Buttons

    @discord.ui.button(label='Join', style=discord.ButtonStyle.grey, disabled=True)
    async def join(self, interaction: discord.Interaction, button: discord.ui.Button):  # noqa
        """Joins the table"""
        await interaction.response.defer()

        if self.table.state != TableState.STOPPED:
            await interaction.followup.send(f'{tick(False)} The table is already running.', ephemeral=True)
            return

        if interaction.user in [player.member for player in self.table.players]:
            await interaction.followup.send(f'{tick(False)} You are already in the game.', ephemeral=True)
            return

        balance = await self.table.economy.get_balance(interaction.user.id, interaction.guild_id)
        if balance.cash < self.table.buy_in:
            await interaction.followup.send(f'{tick(False)} You don\'t have enough money to buy yourself in.\n'
                                            f'You need {coin_emoji} **{self.table.buy_in:,}**.',
                                            ephemeral=True)
            return

        self.table.add_player(interaction.user, stack=self.table.buy_in)

        if len(self.table.players) == 4:
            self.table.start()
            self = TableView(table=self.table)  # noqa
            self.table.pot.amount = self.table.small_blind + self.table.big_blind
    ...
    async def my_hand(self, interaction: discord.Interaction, button: discord.ui.Button):  # noqa
        """Shows the player's hand"""
        player = discord.utils.get(self.table.players, member=interaction.user)
        if not player:
            return await interaction.response.send_message(f'{tick(False)} You are not in the game.', ephemeral=True)

        if self.table.state != TableState.RUNNING:
            return await interaction.response.send_message(f'{tick(False)} The game has not started yet.',
                                                           ephemeral=True)
    ...
    @discord.ui.button(label='Start', style=discord.ButtonStyle.green, disabled=True, custom_id='start_next_round')
    async def start_next_round(self, interaction: discord.Interaction, button: discord.ui.Button):  # noqa
        """Starts the game"""
        await interaction.response.defer()
        ...
        if interaction.user != self.table.host:
            await interaction.followup.send(
                f'{tick(False)} You are not the host of this table.\n'
                f'Please aks {self.table.host.mention} to start the game!', ephemeral=True)
            return

        if len(self.table.players) < 2:
            await interaction.followup.send(f'{tick(False)} You need at least 2 players to start the game.',
                                            ephemeral=True)
            return

        if self.start_next_round.label == 'Next Round':
            await self.table.prepare_next_round()
    ...
    async def fold(self, interaction: discord.Interaction, button: discord.ui.Button):  # noqa
    ...
    async def check_call(self, interaction: discord.Interaction, button: discord.ui.Button):  # noqa
        """Checks or calls"""
        player = await self.get_player(interaction)
        if not player:
            return
        ...
        if player.bet == max_bet:
            ...
            if player.stack < max_bet - player.bet:
                return await interaction.response.send_message(
                    f'{tick(False)} You don\'t have enough chips.', ephemeral=True)
    ...
    async def raise_bet(self, interaction: discord.Interaction, button: discord.ui.Button):  # noqa
        """Raises the bet"""
        player = await self.get_player(interaction)
        if not player:
            return
        ...
        try:
            ...
            return await interaction.response.send_message(f'{tick(False)} Invalid amount.', ephemeral=True)

        if amount > player.stack:
            return await interaction.response.send_message(
                f'{tick(False)} You don\'t have enough chips.', ephemeral=True)

        if not self.table.bet:
            # It's a bet
    ...
    async def all_in(self, interaction: discord.Interaction, button: discord.ui.Button):  # noqa
        """Goes all in"""
        # TODO: Implement side pots functionality to work
        player = await self.get_player(interaction)
        if not player:
            return
    ...
    async def get_player(self, interaction: discord.Interaction) -> Optional[Player]:
        player = self.table.players[self.table.player_index]
        if not player:
            await interaction.response.send_message(f'{tick(False)} You are not in the game.', ephemeral=True)
            return None
...
class LeaveButton(discord.ui.Button):
    """Represents a leave button for the game"""
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.table.state == TableState.RUNNING:
            await interaction.response.send_message(
                f'{tick(False)} The table is currently running, please wait till the game is finished.',
                ephemeral=True)
            return
        ...
        if len(self.table.players) == 1:
            ...
            await interaction.response.send_message(
                f'\N{LEAF FLUTTERING IN WIND} The Poker Table has been closed due to all players leaving.',
                delete_after=10)
            return

        self.view.update_buttons()
        await interaction.response.edit_message(embed=self.table.build_embed(), view=self.view)
        await self.table.message.reply(
            f'\N{LEAF FLUTTERING IN WIND} {interaction.user.mention} has left the table.', delete_after=10)


class AnalysisButton(discord.ui.Button):
    """Represents an analysis button for the game"""

    def __init__(self, table: Table):
        super().__init__(style=discord.ButtonStyle.blurple, label='Show Analysis', emoji='\N{BAR CHART}', row=1)
        self.table: Table = table
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for index, player in enumerate(self.table.players):
            ...
            embed.set_author(name=f'{player.member.display_name} | Seat #{d_index}', icon_url=player.member.avatar.url)
            embed.description = (
                ...
                f'Pre-Flop: Win: **{data[0][0][f'Player {d_index} Win']}**% | Tie: **{data[0][0][f'Player {d_index} Tie']}**%\n'
                f'Flop: Win: **{data[1][0][f'Player {d_index} Win']}**% | Tie: **{data[1][0][f'Player {d_index} Tie']}**%\n'
                f'Turn: Win: **{data[2][0][f'Player {d_index} Win']}**% | Tie: **{data[2][0][f'Player {d_index} Tie']}**%'
            )
            ...
            for i in range(3):
                images.append(
                    render.generate_bar_chart(
                        dict({key: value for key, value in (data[i][1][d_index]).items()}.items()),
                        title=TITLE_MAP.get(i, 'X'),
                        merge=True,
                        to_buffer=False
                    )
                )
            ...
            file = discord.File(
                fp=io.BytesIO(render.add_images_yaxis(images, to_buffer=True)),
                filename=f'analysis_{d_index}.png')

            embed.set_image(url=f'attachment://analysis_{d_index}.png')
            embeds.append(embed)
            files.append(file)

        await self.table.message.reply(embeds=embeds, files=files)
