...
def generate_eq_image(payload: list[float]) -> BytesIO:
...
def _get_gain_y(
        gain: float, *, max_gain=+1.0, min_gain=-0.25, top_margin: int = 0, band_height: int = 0
) -> int:
...
def is_color_dark(rgb_color: tuple[int, int, int]) -> bool:
...
def generate_color_img(rgb: tuple[int, int, int], text: Optional[str] = None) -> BytesIO:
...
def generate_bar_chart(
    ...
    for i in range(image_count):
        ...
        max_label_width = max([get_text_dimensions(label, font=font)[0] for label in subset_data.keys()])
    ...
    if merge:
        ...
        return add_images_yaxis(images, to_buffer=to_buffer)

    return images


def add_images_yaxis(images: list[Image.Image], to_buffer: bool = False) -> Image.Image | bytes:
    """Add the images together among the y-axis."""
...
def create_outlined_rounded_rectangle(
    ...
    with Image.new('RGB', (size[0] + thickness, size[1] + thickness), outline) as outline_image:
        with Image.new('RGB', size, fill) as fill_image:
            outline_image.paste(
                fill_image, (thickness // 2, thickness // 2), create_rounded_rectangle_mask(size, radius))

        return outline_image, create_rounded_rectangle_mask(outline_image.size, radius + (thickness // 2))


def get_dominant_color(image: Image.Image | BytesIO, palette_size=16) -> tuple:
...
def create_image_grid(images: list[Image], grid_size=(2, 2)) -> BytesIO:
    ...
    if len(images) < grid_size[0] * grid_size[1]:
        images += [Image.new('RGB', images[0].size, (0, 0, 0)) for _ in
                   range(grid_size[0] * grid_size[1] - len(images))]
