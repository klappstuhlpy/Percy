...
class FallbackFont:
    ...
    def _prepare(
        self, font: FontT, fallback_loader: Callable[[], FontT], fallback_scale: float, fallback_offset: tuple[int, int]
    ) -> None:
    ...
    def variant(self, *, font: FontT = None, size: int | None = None) -> FallbackFont:  # type: ignore[assignment]
        ...
        new._prepare(
            self.font.font_variant(font=font, size=size),
            self.fallback_loader,
            self.fallback_scale,
            self.fallback_offset,  # type: ignore[union-attr, arg-type]
        )
    ...
    def text(
        self, draw: Draw, xy: tuple[int, int], text: str, fill: ColorT = None, font: FontT = None, *args: Any, **kwargs: Any
        ...
        with Image.open(image) as canvas:
            ...
            if canvas.format in ("PNG", "JPEG", "JPG", "WEBP"):
                ...
                for frame in ImageSequence.Iterator(canvas):
                    ...
                    frames.append(frame.resize([i // 2 for i in frame.size], resample=Image.Resampling.BICUBIC))
