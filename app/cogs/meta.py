...
class GuildUserJoinView(View):
    ...
    async def join_list(self, interaction: discord.Interaction, _) -> None:
        ...
        chunked = [[position, user, user.joined_at] for position, user in enumerate(chunked_users, start=1)]
...
class Meta(Cog):
    ...
    def _snippet_to_codeblock(
        ...
        split_file_contents = file_contents.splitlines()
            ...
            int(end_line) if end_line is not None else (len(split_file_contents) if start_line is None else int(start_line))
        )
            ...
            for match in pattern.finditer(content):
                try:
                    ...
                    log.log(
                        ...
                        error.status,
                        error_message,
                        error.request_info.real_url.human_repr(),
                    )
    ...
    async def create_gist(
        ...
        *,
        ...
        if description:
            data["description"] = description  # type: ignore
    ...
    async def solved(self, ctx: Context) -> None:
        ...
        if can_close_threads(ctx):
            ...
            if confirm:
                ...
                await self.mark_as_solved(ctx.channel, ctx.channel.owner or ctx.user)  # type: ignore
    ...
    async def send_raw_content(ctx: Context, message: discord.Message, json: bool = False) -> None:
        ...
        for field in ("embeds", "attachments"):
            ...
            for current, item in enumerate(data, start=1):  # type: ignore
    ...
    @describe(message="The message to get the raw API response for.", to_json="Whether to send the response in JSON format.")
    ...
    async def featureinfo(self, ctx: Context, guild_id: str | None = None) -> None:
        ...
        embed = discord.Embed(title="Guild Features", timestamp=discord.utils.utcnow(), color=helpers.Colour.white())
    ...
    async def quote(
        ...
        if not user or not message:
            await ctx.send_error(
                "You must specify a user and a message, reply to a message or provide a message_id to quote."
            )
            return
    ...
    async def appinfo(self, ctx: Context, *, app_id: str) -> None:
        ...
        embed = discord.Embed(title=f"{application.name} (ID: {application.id})", description=application.description)
        if application.icon:
        ...
        if application.permissions:
            ...
            embed.add_field(
                name="Permissions", value=f"[`{perms}`](https://discordapi.com/permissions.html#{perms})", inline=False
            )
        ...
        if assets:
            embed.add_field(name="Assets", value="\n".join(f"[{asset.name}]({asset.url})" for asset in assets), inline=False)
    ...
    @group(
        "prefix", description="Manages or show the server's custom prefixes.", invoke_without_command=True, guild_only=True
    )
    async def prefix(self, ctx: Context) -> None:
        """Manages the server's custom prefixes."""
        config: GuildConfig = await self.bot.db.get_guild_config(guild_id=ctx.guild.id)  # type: ignore[misc, union-attr]
    ...
    @permissions.command("user", description="Shows a member's permissions in a specific channel.", guild_only=True)
