...
class Snekbox(Cog):
    ...
    async def continue_job(self, ctx: EvalContext, response: Message, job_name: str) -> EvalJob | None:
        ...
        with contextlib.suppress(NotFound):
            try:
                _, new_message = await self.bot.wait_for("message_edit", check=_predicate_message_edit, timeout=REDO_TIMEOUT)
                ...
                code = await self.get_code(new_message, ctx.command)  # type: ignore
