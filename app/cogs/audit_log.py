...
class AuditLog(Cog):
    ...
    @classmethod
    def to_title(cls, string: str | None = "") -> str:
        return string.replace("_", " ").title()

    @classmethod
    def resolve_target(cls, target: PossibleTarget | None, *, raw: bool = False) -> str | None | Any:
    ...
    def get_permissions_changes(cls, entry: discord.AuditLogEntry, change_type: str) -> str:
        ...
        with suppress(AttributeError, TypeError):
            for b, a in zip(getattr(entry.before, change_type), getattr(entry.after, change_type)):
                if b[1] != a[1]:
                    changes.append(f"{cls.resolve_target(entry.extra)}: **{b[0]}** `{b[1]}` -> `{a[1]}`")  # type: ignore
        return "\n".join(changes)
    ...
    def get_action_color(cls, action: discord.AuditLogAction) -> int:
        ...
        if category is None:
            return 0x2F3136
        elif category.value == 1:  # type: ignore
            return 0x3BA55C
        elif category.value == 2:  # type: ignore
            return 0xEC4245
        elif category.value == 3:  # type: ignore
            return 0xFAA61A
        else:
            return 0x000000
    ...
    async def on_audit_log_entry_create(self, entry: discord.AuditLogEntry) -> None:
        config = await self.bot.db.get_guild_config(entry.guild.id)  # type: ignore
        ...
        action_event_type = self.to_title(entry.action.name)
        ...
        target = self.resolve_target(entry.target, raw=True)  # type: ignore
        by = self.resolve_target(getattr(entry, "user", "N/A"))  # type: ignore
