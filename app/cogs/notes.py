...
class UserNotes(Cog, name="Notes"):
    ...
    async def notes_autocomplete(self, interaction: discord.Interaction, current: str) -> list[
        ...
        return [
            app_commands.Choice(
                name=get_shortened_string(length, start, f"{note['topic']} - {note['content']}"), value=note["id"]
    ...
    async def note_add(
        self, interaction: discord.Interaction, content: str, topic: str | None = None, expiration: Time | None = None
    ) -> None:
        """Take a note."""
        if len(content) > 2000:
    ...
    async def note_list(self, interaction: discord.Interaction) -> None:
        ...
        embed = discord.Embed(title="Your Notes", color=helpers.Colour.white(), timestamp=discord.utils.utcnow())
    ...
    @app_commands.autocomplete(note_id=notes_autocomplete)  # type: ignore
    ...
    async def note_view(self, interaction: discord.Interaction, note_id: int) -> None:
        ...
        embed = discord.Embed(
            title=f"Note {note.id}", description=note.content, color=helpers.Colour.white(), timestamp=note.created_at
        )
        footer = []
        if note.topic:
    ...
    @describe(
        note_id="The ID of the note to edit.", content="The new content of the note.", topic="The new topic of the note."
    )
    @app_commands.autocomplete(note_id=notes_autocomplete)  # type: ignore
    async def note_edit(
        self, interaction: discord.Interaction, note_id: int, content: str | None = None, topic: str | None = None
    ) -> None:
        """Edit a note."""
        ctx = await self.bot.get_context(interaction)
        note = await self.get_note(note_id, ctx.author.id, with_timer=False)
        ...
        await note.update(content=content or note.content, topic=topic or note.topic)
