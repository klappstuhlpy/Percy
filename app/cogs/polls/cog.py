...
class Polls(Cog):
    ...
    def __init__(self, bot: Bot) -> None:
        ...
        bot.add_dynamic_items(PollEnterButton, PollEnterSelect, PollClearVoteButton, PollInfoButton, PollRolePingButton)
    ...
    async def poll_id_autocomplete(self, interaction: discord.Interaction, current: str) -> list[Choice[str | int | float]]:
    ...
    async def end_poll(self, poll: Poll, /) -> int | None:
        ...
        if (poll.message_id is not None and poll.message is MISSING) or (
            poll.kwargs.get("ping_message_id") is not None and poll.ping_message is MISSING
        ):
    ...
    @group("polls", aliases=["poll"], description="Group command for managing polls.", guild_only=True, hybrid=True)
    ...
    @polls.command(
        "delete", description="Deletes a poll question.", guild_only=True, user_permissions=PermissionTemplate.mod
    )
    ...
    @polls.command("search", description="Searches poll questions. Search by ID, keyword or flags.", guild_only=True)
    ...
    @app_commands.autocomplete(poll_id=poll_id_autocomplete)  # type: ignore
    async def polls_search(self, ctx: Context, poll_id: int | None = None, *, flags: PollSearchFlags) -> None:
        ...
        if poll_id:
            ...
            if flags.showextrainfo and ctx.channel.permissions_for(ctx.user).manage_messages:
                embed = discord.Embed(title=f"#{poll.kwargs.get('index')}: {poll.question}", description=poll.description)

                embed.add_field(
                    name="Choices", value="\n".join(f"{v['value']}" for v in poll.to_fields(extras=False)), inline=False
                )
                ...
                embed.add_field(
                    name="Poll ends" if running else "Poll finished", value=discord.utils.format_dt(poll.expires, "R")
                )
            ...
            text = ["**Filter(s):**", f"Sorted by: **{flags.sort.lower()}**"]

            if flags.active:
                text.append("Running: **True**")

            records = await self.bot.db.polls.search_for_guild(ctx.guild.id, sort=flags.sort, active=flags.active)
    ...
    @polls.command("history", description="Shows the vote history of a user for polls.", guild_only=True)
    ...
    async def polls_history(self, ctx: Context, member: discord.Member | None = None) -> None:
        ...
        class FieldPaginator(BasePaginator[Poll]):
            async def format_page(self, entries: list[Poll], /) -> discord.Embed:
                embed = discord.Embed(
                    title=f"Poll History for {member}", colour=helpers.Colour.white(), timestamp=discord.utils.utcnow()
                )
    ...
    async def polls_config(self, ctx: Context, *, flags: PollConfigFlags) -> None:
        ...
        if all(x is None for x in [flags.channel, flags.reason_channel, flags.ping_role]):
            embed = discord.Embed(
                title="Poll Configuration", colour=helpers.Colour.white(), timestamp=discord.utils.utcnow()
            )
            embed.add_field(name="Poll Channel", value=f"<#{config.poll_channel_id}>" if config.poll_channel_id else "N/A")
            embed.add_field(
                name="Poll Reason Channel",
                value=f"<#{config.poll_reason_channel_id}>" if config.poll_reason_channel_id else "N/A",
            )
            embed.add_field(name="Poll Role", value=f"<@&{config.poll_ping_role_id}>" if config.poll_ping_role_id else "N/A")
            embed.set_footer(text='Use "/polls config" to change the configuration.')
            await ctx.send(embed=embed)
        else:
            if flags.reset:
                form = {"poll_channel_id": None, "poll_reason_channel_id": None, "poll_ping_role_id": None}
