...
per-guild gallery that Percy uploads poll banners into. Percy is the sole holder
of the klappstuhl.me ``images:guild`` key; nothing here touches Percy's own
database, so there is no cache to invalidate.
"""
...
def _client_or_503(bot) -> KlappstuhlMeClient:
    ...
    if client is None or not client.api_key:
        raise HTTPException(
            ...
            detail="the image hoster is not configured (KLAPPSTUHL_ME_API_TOKEN unset)",
        )
    return client
...
async def list_gallery(guild: GuildDep, bot: BotDep) -> dict:
    ...
    try:
        return await client.list_guild_images(guild.id)
...
async def upload_gallery(
    ...
    if result.get("errors") or result.get("infected"):
        raise HTTPException(
...
async def delete_gallery_image(guild: GuildDep, bot: BotDep, image_id: str) -> dict:
    ...
    client = _client_or_503(bot)
    try:
        return await client.delete_guild_image(guild.id, image_id)
    except Exception as exc:
